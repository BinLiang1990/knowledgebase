...
for index, item in enumerate(payload.items):
    try:
        ...
        # 兜底：任何未预期的异常也不能让整批中断，否则前面已经成功、SAVEPOINT
        # 已 RELEASE 的 item 会随 session 从未 commit 而全部丢失——见下方
        # 对抗式审查第二处阻塞级问题的说明。
        results.append(_failed_result(index, item.title, "内部错误，请重试该条"))
        continue
...
第二处阻塞级问题：单条创建接口的 `_raise_if_duplicate_title`（`knowledge_point.py:41-45`）对非 1062（`_MYSQL_ER_DUP_ENTRY`）的 `IntegrityError` 会原样 `raise exc`；如果批量循环照搬这个函数，一次意外的（非标题重复）约束错误会重新抛出、不被任何 `except` 兜住、终止整个循环，同样会打穿"部分成功"承诺。批量场景**不能复用会重新 `raise` 的 `_raise_if_duplicate_title`**，必须用上面这个只返回文案、不重新抛出的 `_batch_item_failure_reason`；并且额外加一层 `except Exception` 兜底（覆盖标题重复之外的任何意外失败），确保无论发生什么，当前 item 至多标记为失败，绝不会让循环提前终止、连累其他已成功的 item。
...
进入 handler 之后，每个 item 的失败原因分两类（对应 §4.2 骨架代码里的两层 `except`）：
1. **标题重复**（预期的业务失败，`_batch_item_failure_reason` 命中 1062 时的固定文案）——绝大多数失败会落在这一类。
2. **兜底的"内部错误，请重试该条"**（`except Exception` 兜住的任何未预期异常，包括非 1062 的 `IntegrityError`）——不细分具体原因，只保证不让这一条的意外失败传播出去连累其他 item。这类失败在正常运行下应该极少出现；如果测试或线上观察到大量落在这一类，说明有更根本的问题需要单独排查，不属于"标题重复"这种预期的业务校验失败。
