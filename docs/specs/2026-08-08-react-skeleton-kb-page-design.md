...
**Codex 外门第二轮认为"停用后知识库应该从列表消失",本设计不采纳,理由已有实证。** §4.1"停用"行原文"知识库列表不再展示"字面上确实这么写,但这条 issue 明确要求"知识库列表/新增/编辑/停用交互与 `frontend-mock/kb-list.html` 一致"——而 `kb-list.html` 本身就是**唯一**的"知识库列表"页,它的 `renderKbList()` 直接 `getKnowledgeBases()` 拿全部知识库(没有任何 status 过滤),对已停用的库照常展示、附带"启用"操作,没有另外一个隐藏的、只显示 active 的"选择器"视图。这和 issue #2 后端设计文档(`2026-08-07-knowledge-base-api-design.md` §3 分组 1)已经审过的结论完全一致："停用后不再展示"针对的是**面向使用者的知识库选择器**(比如写答案时选知识库),不是**本页面这种管理接口**——第三方/管理员正是通过这个页面把已停用的库重新启用,如果列表里看不到已停用的库,这个操作根本没有入口。保持现状(展示全部、用状态列区分),不是漏改,是两次独立评审都已经落地的同一个决定。
