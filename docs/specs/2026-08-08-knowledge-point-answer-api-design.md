...
Codex 外门审查第三轮又发现两个问题,均已修正：

- **量级检查本身要在"可能很贵的转换"之前做,不能在之后。** `"1e1000000000"` 这种字符串本身很短,但 `Decimal` 解析后判断"是不是整数"再调 `int()`,会真的把这个十亿位数字物化成一个巨大的 Python 整数对象——一个几十字节的请求体就能触发一次昂贵的内存分配。修正：用 `Decimal.adjusted()`(只读最高位数字所在的位置,不物化任何数字,开销恒定)在调用 `int()` 之前先做一次量级粗筛,粗筛通过了才进入"转成整数/判断是否安全"的正常流程。
- **小数精度超出 float 能精确表示的范围时,不能静默四舍五入合并。** `float()` 转换只有约 15–17 位有效数字的精度,`"1.0000000000000000000000001"` 和 `"...0002"` 这种超出精度范围的小数会被精度损失成同一个 float,进而落到同一个 `coord_hash`,变成"两个不同取值被误判成同一条链"。修正：转换成 float 后,用 `repr(float_value)`(Python 保证是能精确还原该 float 的最短字符串)重新解析回 `Decimal` 跟原始输入比较,不完全一致就拒绝——不静默丢精度,而是明确告诉调用方这个数值精度超出支持范围。
...
**"写一条答案"和"迁移到一个新条件组合"都要检查目标条件组合是不是一条已撤回的链,拒绝直接写入。** 最初的实现漏了这一步(Codex 外门审查第二轮发现)：`POST .../answers` 只要 `coord` 归一化通过就无条件插入一行 `revoked=False` 的新版本,如果这个 `coord_hash` 恰好是一条已经被撤回的链(不管是通过本 issue 的迁移撤回,还是未来 #10 的手动撤回),就会让这条链出现"部分行撤回、部分行不撤回"的矛盾状态,等价于悄悄把一条已撤回的链复活了。编辑答案触发迁移时,如果新条件组合恰好撞上另一条已经被撤回的链,同样的问题会发生。修正：两处都先查"目标 `coord_hash` 在这个知识点下是否已存在任何 `revoked=True` 的行",命中就拒绝(P0 阶段没有"恢复已撤回链"的操作,这个状态在本轮范围内无法被正常修复,所以直接拒绝而不是尝试特殊处理)。
