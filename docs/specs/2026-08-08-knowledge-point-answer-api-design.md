...
Codex 外门审查第四轮又发现一个问题,已修正,且这次的根因和前几轮不一样：**上面所有针对字符串输入的精确性检查,都建立在"我们拿到的是原始文本"这个前提上;但如果客户端把 `coord` 里的数值写成裸的 JSON 数字(不带引号)而不是字符串,请求体在到达 `normalize_coord` 之前就已经被标准 JSON 解析器转换成 Python `float` 了——这一步精度损失发生在我们的代码运行之前,原始的完整数字文本早就丢了,压根没有机会再用 `Decimal`/`repr` 去核对**。比如 `9007199254740993.0` 这个裸数字字面量,解析出来就已经是 `9007199254740992.0`,和真的输入 `9007199254740992.0` 完全无法区分。修正思路不是"想办法恢复精度"(做不到,信息已经丢了),而是"能确定这个值不可信时就拒绝"：float 只能精确表示到 `2**53` 为止的整数,超过这个量级的、看起来是整数的裸浮点数直接拒绝,提示调用方改用字符串形式传入(那条路径可以精确解析)。裸浮点数的小数部分不做类似拒绝——那部分从 JSON 解析的那一刻起就已经是"能表示多精确就表示多精确"的最终值,没有进一步的精度损失风险,拒绝它没有实际意义。
...
编辑目标是否已被撤回不再是一道拒绝闸门(见下方 2026-08-10 修订)——历史上这里曾要求编辑目标必须是`revoked=False`,理由是"P0 阶段没有复活操作,允许追加版本会做出一条逻辑上不存在的半撤回状态"；这个理由本身仍然成立,只是解法从"拒绝"换成了"复活整条链再追加",半撤回状态照样避免。

**"写一条答案"和"迁移到一个新条件组合"都要检查目标条件组合是不是一条已撤回的链。** 最初的实现漏了这一步(Codex 外门审查第二轮发现)：`POST .../answers` 只要 `coord` 归一化通过就无条件插入一行 `revoked=False` 的新版本,如果这个 `coord_hash` 恰好是一条已经被撤回的链(不管是通过本 issue 的迁移撤回,还是未来 #10 的手动撤回),就会让这条链出现"部分行撤回、部分行不撤回"的矛盾状态,等价于悄悄把一条已撤回的链复活了。编辑答案触发迁移时,如果新条件组合恰好撞上另一条已经被撤回的链,同样的问题会发生。当时的修正：两处都先查"目标 `coord_hash` 在这个知识点下是否已存在任何 `revoked=True` 的行",命中就拒绝——这个"拒绝"分支已经在 2026-08-10 的修订里换成"整条链一并复活"，见下方说明。

#### 2026-08-10 修订：撤回不再是终态，改为"写入即复活整条链"

用户实测发现：往一个曾经被撤回的条件组合（如 `场景=农业赛道`）写新答案，被 400 拒绝，且当时的设计（连同 PRD §8 的"撤回的撤回"非目标）明确把"复活已撤回链"排除在 v1 范围外——用户认为这个限制不对，要求放开。四处拒绝分支全部改为"先复活整条链（把该 `coord_hash` 下所有行的 `revoked`/`revoked_at`/`revoked_by`/`revoke_reason` 清空），再照常写入新版本"：`create_answer` 直接写入、`edit_answer` 的迁移目标链、`edit_answer` 直接编辑一条已撤回的答案、`promote_to_default` 的默认链目标。

**为什么"复活整条链"而不是"只让新插入的这一行不撤回"**：本节上面这段警告过的"部分行撤回、部分行不撤回"矛盾状态，是当年拒绝方案要避免的唯一理由，不是"撤回不能被撤销"这条业务规则本身站得住脚——只要新方案同样保证"整条链的 `revoked` 永远一致"，这条不变量（PRD §6 规则 #4）就没被打破，之前的拒绝就不再是必需的。

**编辑触发迁移时,如果被编辑的答案自己所在的旧链本来就已经撤回**：旧链撤回的那条 `UPDATE`（把 `revoked=True` 连同新的 `revoked_at/revoked_by/revoke_reason` 一起写回旧链)必须只在旧链当前是`revoked=False` 时才生效(`WHERE revoked = 0`)，否则会用这次迁移的原因覆盖掉旧链原本真实的撤回原因/时间——这不是这次修订新引入的场景，但只有编辑已撤回答案被允许之后才会真正触发，属于同一次修订要一并处理的边界。
...
### 3.8.1 `answer.note` 列类型缺陷（issue #1 遗留,Codex 第五轮发现）

issue #1 的迁移把 `answer.content` 定为 `LONGTEXT`(无长度上限),但 `note` 当时定成了普通 `Text`(MySQL `TEXT`,上限 65,535 字节)。§4.5 校验规则原文"content、note 暂不设长度上限"同时覆盖两个字段,`note` 的列类型和这条规则矛盾——传一个超过 65KB 的 `note` 会在 `db.commit()` 时炸出一个未处理的原始数据库异常(500),而不是本该有的"确实不限长度"的行为。修正：新增迁移 `0002`,把 `answer.note` 列类型改成 `LONGTEXT`,ORM 模型同步更新。这是本 issue 范围内第一次需要新增迁移文件(而不是只操作已有表结构),迁移在"表是空的"前提下可逆(`downgrade` 改回 `TEXT`)。

这条迁移暴露了一个测试基础设施的隐藏耦合：验证"note 能存下超过 65535 字节"这个行为的测试,写入数据后如果不清理,会导致该测试自己的 `migrated_schema` fixture 在 teardown 阶段（downgrade 到 base,途中要经过 `0002 -> 0001` 把列窄化回 `TEXT`）失败——MySQL 拒绝把一个存有超长数据的列改窄,这是数据库层面完全正确的行为,不是 bug,但它会让那次失败的 teardown 把数据库留在一个介于两个迁移版本之间的破损状态,直接连带拖垮后面所有测试的启动（因为它们各自的 `migrated_schema` fixture 一上来也要先 downgrade 到 base,而此时的 DB 已经不是任何一个正常状态）。修正方式不是改迁移本身(生产环境下"降级时因为存在超限数据而失败"是期望行为,不应该被悄悄绕过或改成静默截断),而是让这个测试在验证完成后主动清空自己写入的超长数据,保证 teardown 能顺利把 DB 降回空表。

### 3.8.2 Kimi 终审发现的问题

四项 P2,无 P1 阻塞项：

1. **并发迁移竟态——不修,记录为已知的、被 PRD 明确推迟的问题。** Kimi 指出两个并发的编辑请求可能在"检查链是否已撤回"和"真正撤回/插入"之间出现竟态,理论上能让一条已撤回的链被复活。这条和 §4.10 已经写明的决策直接对应："并发写冲突控制(🟢 P2,待补充)：同一知识点、同一条件组合(coord)被并发编辑/撤回时的加锁或冲突处理策略,v1 暂不做特殊处理...不阻塞本轮 P0 交付"。本 issue 不引入行级锁(`with_for_update`)或乐观锁,按 PRD 既定决策处理,留给后续视接入情况评估。
2. **`list_knowledge_points` 的按知识库统计在用答案数查询缺一个合适的索引。** 现有两个索引都以 `knowledge_point_id` 开头,查询条件是 `knowledge_base_id + revoked`,数据量大了会退化成全表扫描。修正：新增迁移 `0003`,加一个 `(knowledge_base_id, revoked, knowledge_point_id)` 复合索引,ORM 模型同步更新。
3. **数字字符串在物化成 `Decimal`之前,只挡了"指数过大",没挡"字符串本身过长"。** `"1." + "0"*1000000` 这种全是尾随零的字符串,`adjusted()` 判断出来是 `0`(就是 1.0 而已),能绕过第三轮加的量级粗筛,但 `Decimal(text)` 本身处理这么长的字符串还是有开销——一个很小的请求体就能触发一次不必要的大字符串解析。修正：在调用 `Decimal()` 之前先加一道字符串长度上限检查(100 字符,远超这个系统里任何合法数值需要的长度)。
4. **测试基础设施的"隐藏耦合"。** 上面 3.8.1 提到的"测试自己清理超长 note"那个方案,Kimi 指出这是个隐患——以后任何一个新测试写了超长 `note` 却忘了清理,就会重演同样的级联失败。修正为更彻底的方案：把清理逻辑从"每个测试自己负责"移到 `migrated_schema` fixture 的 `finally` 块本身,在 downgrade 之前先无条件 `TRUNCATE TABLE answer`,不管任何测试有没有记得清理,teardown 都不会被卡住。原来那个测试里手动清理的代码相应删除,避免给后人一个"还需要手动清理"的错误示范。
