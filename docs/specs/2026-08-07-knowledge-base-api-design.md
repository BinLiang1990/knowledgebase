...
3. **重名校验的范围和时机。** §4.1 校验规则原文"与现有任意知识库（含已停用的）完全重复"，范围覆盖 active + deprecated。~~完全匹配（大小写、空白都算数,不做归一化/模糊匹配)~~ ——**此处最初的判断是错的，已在 Codex 外门审查中纠正**：`knowledge_base.name` 列用的是 issue #1 定下的 `utf8mb4_0900_ai_ci` collation，大小写/重音不敏感是 MySQL 比较和唯一索引本身自带的行为，不是应用层决定的。重新审视后认为这个行为是"完全重复"的合理落地——它顺带防住了"FAQ"和"faq"这种容易让第三方混淆的近似重复，改成大小写敏感需要新迁移去改列的 collation，对已上线的 issue #1 schema 动刀，收益不足以覆盖风险，因此保留现状，只更正文档描述、补充测试固化这个行为。应用层先查一次给出干净的错误提示,同时数据库唯一索引 `uq_knowledge_base_name`（issue #1 已建）兜底防止两个并发创建请求的竟态漏检——命中该约束时同样转换成同一条 `BusinessError`,不让原始 `IntegrityError` 直接 500 泄露给客户端。修改名称时,校验要把"当前这条记录自己"排除在外,否则改描述不改名字都会被误判成"和自己重名"。
...
1. **PATCH 无法把已有 `description` 清空成 `null`。** `payload.description is not None` 分支把"字段未传"和"字段显式传了 `null`"混为一谈，导致清空描述这个合法请求被静默忽略。**修正**：改用 `payload.model_fields_set` 判断字段是否被客户端显式携带（哪怕值是 `null`），而不是看字段的值本身。
2. **重名校验的大小写/重音敏感性文档描述有误，见上方第 3 点的更正。**
