...
### 5.2.1 Codex 外门审查发现的问题

- **[P1] 单点解析接口没有拦截已软删除的知识点。** §4.4"删除"行原文："删除后不出现在知识点列表**与查询结果**中"——本设计最初只让"列表接口"默认排除已删除知识点(通过 `status=active` 默认值),完全没考虑单点解析接口本身也是一种"查询结果",而它没有任何状态过滤,只要 `kp_id` 存在就会照常解析并返回其保留的答案内容,直接违反这条规则。修正：解析前先检查 `kp.status`,已删除则直接返回 `status="none", answer=None`(不是 404——这个接口自己的约定是"知识点只要真实存在就返回 200 + 一个 status 字段",对调用方而言"已删除"在语义上等价于"这个知识点当前没有可查询的答案",不是"这个 ID 不存在")。列表接口 `?status=deleted` 场景不受影响——那是管理员主动查看回收站,不是这条规则要防的"外部查询意外看到已删除内容"。
- **[P2] 关键字子串匹配没有转义 SQL LIKE 的通配符。** `keyword` 里如果包含字面的 `%` 或 `_`,SQLAlchemy 的 `.contains()` 默认会把它们当成 LIKE 通配符而不是普通字符,导致匹配结果和 JS 的 `.includes()`(纯字面子串比较)不一致。修正：`.contains(keyword, autoescape=True)`。

**第二轮又发现一个 [P1],经核实后判断为不适用,不修——记录理由而不是直接照做：** Codex 指出 `coord.py` 空字符串归一化这条修正只影响"以后新写入/新查询"的数据,如果数据库里已经存在用旧逻辑写入的 `coord={"tenant":""}` 历史答案,这些行的 `coord`/`coord_hash` 不会被回溯更新,会继续被当成一个独立的"tenant=空字符串"分组,和新写入的默认分组(`coord={}`)并存,理论上确实会导致排序/分组不一致。**这个顾虑对一个已经有真实存量数据的线上系统是成立的,但对本项目当前的实际状态不成立**：这是一个还没有任何第三方接入、还在 P0 开发阶段的全新系统,实测当前真实数据库里只有 `alembic_version` 这一张表(`answer` 表本身都不存在,因为每次手动验证后都会 downgrade 回 base)——不存在任何需要回填的历史数据。因此不新增数据迁移脚本去修补一个不存在的数据集。如果将来这套系统已经有真实存量数据、又发生类似的归一化规则变更,正确的做法应该是加一条数据迁移,对存量行重新归一化 + 重算 `coord_hash`,而不是只改代码——这个模式记录下来,供以后类似情况参考,但当前不需要动手做。

### 5.2.2 Kimi 终审发现的问题

三项 P2/minor,无 P1 阻塞项：

- **[P2] 三处 `max()` 的 tie-break 元组不够,并列时结果不确定。** `versions`/`groups`/`candidates` 都来自没有 `ORDER BY` 的 SQL 查询,行的返回顺序取决于 MySQL 的执行计划,不是稳定的；当 `(effective_time, created_at)` 或 `(spec, weight, effective_time, created_at)` 完全并列时,Python `max()` 的"稳定取第一个最大值"特性会让结果依赖于这个不确定的行顺序,同一个查询在不同时刻可能给出不同答案。修正：三处 tie-break 元组统一补上 `answer.id` 作为最后一级,保证任何情况下都有一个确定的胜者。
- **[P2] `keyword` 参数没有长度上限,直接拼进 SQL LIKE,是一个简单的 DoS 面。** 修正：`Query(max_length=255)`,对齐 `knowledge_point.title` 自己的列宽。
- **[minor] 空白字符组成的 keyword(如 `"   "`)会静默失效。** `if keyword:` 判断的是原始字符串(非空格字符串本身是"真"),但实际用于过滤的是 `.strip()` 之后的结果——如果 strip 后是空字符串,`contains("")` 会匹配所有标题,等价于"没有过滤",但代码意图上看不出这是刻意的。修正：判断条件也改成看 strip 之后的值。
